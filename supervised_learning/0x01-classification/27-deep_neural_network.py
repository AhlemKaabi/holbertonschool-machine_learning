...
class DeepNeuralNetwork:
    ...
    def forward_prop(self, X):
        ...
        # use the softmax function for the output layer!
        self.cache['A0'] = X
        for l_ in range(self.L):
            W_l = self.weights['W' + str(l_ + 1)]
            b_l = self.weights['b' + str(l_ + 1)]
            A_prev = self.cache['A' + str(l_)]
            Z = np.matmul(W_l, A_prev) + b_l
            if l_ := self.L - 1:
                # use a sigmoid function for hidden layers!
                A_l = 1 / (1 + np.exp(-Z))
            else:
                # use the softmax function for the output layer!
                term = np.exp(Z)
                A_l = term / np.sum(term)
    ...
    def cost(self, Y, A):
        """
        Method:
            The cost of the model use a cross entropy function
        Args:
        ...
        """
        classes, m = Y.shape
        # we will use a cross entropy loss
        # https://rdipietro.github.io/friendly-intro-to-cross-entropy-loss/
        loss = np.matmul(Y, np.log(A).T)
        cost = -np.sum(loss) / m
        return cost
    ...
    def save(self, filename):
        ...
        # saving a program’s state data to disk so that it can carry on
        # where it left off when restarted (persistence)
